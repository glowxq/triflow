...
function normalizeTabbarItem(item: CustomTabBarItem): CustomTabBarItem {
  return {
    ...item,
...
export function resetTabbarList(items: CustomTabBarItem[]) {
  tabbarList.splice(0, tabbarList.length, ...items.map(normalizeTabbarItem))
}
